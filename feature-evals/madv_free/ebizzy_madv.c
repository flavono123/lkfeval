...
usage(void)
{
	fprintf(stderr, "Usage: %s [options]\n"
		...
		"-f\t\t MADV_FREE flag for madvise (MADV_DONTNEED by default)",
        cmd);
	exit(1);
}
...
start_threads(void)
{
    ...
    if (madv_flag)
        strcpy(flag, "MADV_FREE");
	printf("%s_records/s\t%u \n",
	       flag, (unsigned int) (((double) records_read)/elapsed));

	usr_time = difftimeval(&end_ru.ru_utime, &start_ru.ru_utime);
	sys_time = difftimeval(&end_ru.ru_stime, &start_ru.ru_stime);

	printf("%s_real\t%5.2f \n", flag, elapsed);
	printf("%s_user\t%5.2f \n", flag, usr_time.tv_sec + usr_time.tv_usec/1e6);
	printf("%s_sys \t%5.2f \n", flag, sys_time.tv_sec + sys_time.tv_usec/1e6);
}
